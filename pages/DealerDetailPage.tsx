...
const deriveDealerStatus = (dealer: Dealer): DealerStatus => {
    if (dealer.isDeleted) {
        return 'deleted';
    }

    const status = dealer.status as DealerStatus | undefined;
    if (status) {
        if (status === 'approved' && dealer.isActive === false) {
            return 'inactive';
        }
        return status;
    }
...
const DealerDetailPage: React.FC = () => {
    ...
    const dealerStatus = deriveDealerStatus(dealer);

    const isDeleted = dealerStatus === 'deleted';
    const isApproved = !isDeleted && dealerStatus === 'approved' && dealer.isActive !== false;
    const isPending = !isDeleted && dealerStatus === 'pending';
    const isInactive = !isDeleted && dealerStatus === 'inactive';
    const isRejected = !isDeleted && dealerStatus === 'rejected';
                            ...
                            <div className="flex flex-wrap gap-3">
                                {!isDeleted && (
                                    isApproved ? (
                                        ...
                                        <span className="inline-flex items-center gap-2 rounded-full bg-rose-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-rose-200">
                                            <ShieldAlert size={14} />
                                            {t('dealerDetails.rejected', { defaultValue: 'Dealer rejected' })}
                                        </span>
                                    ) : isInactive ? (
                                        <span className="inline-flex items-center gap-2 rounded-full bg-amber-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-amber-300">
                                            <ShieldAlert size={14} />
                                            {t('dealerDetails.inactive', { defaultValue: 'Dealer inactive' })}
                                        </span>
                                    ) : isPending ? (
                                        <span className="inline-flex items-center gap-2 rounded-full bg-amber-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-amber-300">
                                            <ShieldAlert size={14} />
